"""Multimodal fusion net (IMU Conv1D + VideoMAE-frame Transformer), subject-disjoint 5-fold, test-format windows.
...
fold_of = {s: i % FOLDS for i, s in enumerate(perm)}; fold_sec = np.array([fold_of[s] for s in sbj])
oof = np.full((N, 4, NC), np.nan, np.float32); test = np.zeros((len(tm), NC), np.float32); cv = {}
te_secs = np.arange(len(tm));
for k in range(FOLDS):
    tr = fold_sec[rows_sec] != k; va = ~tr
    trs, trl = rows_sec[tr], rows_limb[tr]
    cnt = np.bincount(y_sec[trs], minlength=NC); keep_null = int(3 * np.median(cnt[1:]))
    nmask = y_sec[trs] == 0; nidx = np.where(nmask)[0]
    sel = np.ones(len(trs), bool)
    if len(nidx) > keep_null: sel[rng.choice(nidx, len(nidx) - keep_null, replace=False)] = False
    trs, trl = trs[sel], trl[sel]
    cnt2 = np.bincount(y_sec[trs], minlength=NC).astype(np.float64); w_cls = torch.tensor((cnt2.mean() / np.maximum(cnt2, 1)) ** 0.5, dtype=torch.float32, device=dev)
    ...
    log(f"fold {k}: train rows {len(trs)} (null kept {keep_null}), val rows {va.sum()}, steps/epoch {len(dl)}")
    for ep in range(EPOCHS):
        ...
        for x, v, l, yy in dl:
            ...
            tl += loss.item(); nb += 1
            if SMOKE and nb >= 30: break
        p = predict(model, rows_sec[va], rows_limb[va])
        f1 = f1_score(y_sec[rows_sec[va]], p.argmax(1), average="macro")
        log(f"fold {k} ep {ep}: loss {tl/nb:.4f} val row-F1 {f1:.4f}")
    oof[rows_sec[va], rows_limb[va]] = p
    test += predict(model, te_secs, limb_te, imu_src=XI_te, vid_src=VID_te) / FOLDS
    cv[k] = dict(f1_rows=float(f1), n_tr=int(len(trs)), n_va=int(va.sum()))
    torch.save(model.state_dict(), f"{OUT}/model_f{k}.pt")
valid = ~np.isnan(oof[:, :, 0]); rl = np.array([rng.choice(np.where(v)[0]) if v.any() else -1 for v in valid]); m1 = rl >= 0
cv["oof_f1_single_limb"] = float(f1_score(y_sec[m1], oof[np.arange(N)[m1], rl[m1]].argmax(1), average="macro"))
log("OOF single-limb F1", cv["oof_f1_single_limb"])
np.save(f"{OUT}/oof.npy", oof); np.save(f"{OUT}/test.npy", test); json.dump(cv, open(f"{OUT}/cv.json", "w"), indent=1)
pd.DataFrame({"id": tm.id, "target_feature": test.argmax(1)}).to_csv(f"{OUT}/sub_raw_argmax.csv", index=False)
log("DONE")
