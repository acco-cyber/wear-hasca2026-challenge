"""Build simulated link structure for EXTRA sessions (neither eval nor scorer-fit) with the frozen scorer.
Limbs seeded with a fresh np.random.RandomState(1) per session.  -> exp/decoder/extra_struct.pkl
python build_extra.py --sessions sbj_2,sbj_8,sbj_13,sbj_17
"""
...
WORK = r"E:\Claude code\wear\work"; OUT = r"E:\Claude code\wear\exp\decoder\extra_struct.pkl"

def main():
    ap = argparse.ArgumentParser(); ap.add_argument("--sessions", default="sbj_2,sbj_8,sbj_13,sbj_17"); a = ap.parse_args()
    ...
    for s in a.sessions.split(","):
        ...
        out[s] = dict(a=aa, b=bb, n=n, y=meta.y.to_numpy()[aa:bb], limb=limb, cand=cand, lo=lo, succ0=succ0, sc=scs, Lm=Lm)
